...
export async function POST(request: Request) {
  try {
    ...
    const prompt = `You are an expert legal document analyzer. Analyze this document thoroughly and provide a comprehensive analysis in the following JSON format. Be accurate and helpful.

IMPORTANT: Return ONLY valid JSON, no markdown, no code blocks, just the raw JSON object.

{
  "documentType": "Type of document (e.g., Traffic Violation Notice, Court Summons, Legal Notice, Rental Agreement, etc.)",
  ...
  "sections": [
    {
      "id": "1",
      "title": "Section name/number mentioned in document",
      ...
      "penalty": "What are the potential penalties or consequences",
      "stateNote": "Any state-specific notes or variations"
    }
  ],
  "extractedDetails": {
    "caseNumber": "Case/Challan/Reference number if found",
    "date": "Date mentioned in document",
    "jurisdiction": "Court/Authority name",
...
Analyze the document carefully. If any field cannot be determined from the document, use reasonable defaults or indicate "Not specified" where appropriate. For sections array, include all relevant legal sections mentioned in the document.`
